                ...
                <div>
                  ...
                  <div style={{ borderBottom: "1px solid #e2e8f0", marginBottom: 16, paddingBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 13, color: "#718096" }}>
                      <span>Ì≥Ö Posted: {formatDate(selectedVacancy.created_at)}</span>
                      <span>Ì¥ñ Category: {selectedVacancy.category}</span>
                    </div>
                  </div>
                  <div style={{ lineHeight: 1.8, fontSize: 15, color: "#334155" }}>
                    <p>{selectedVacancy.content}</p>
{selectedVacancy.file_url && <div style={{ marginTop: 16 }}><a href={`http://127.0.0.1:8000${selectedVacancy.file_url}`} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ display: "inline-block", fontSize: 13, padding: "6px 16px", textDecoration: "none" }}>Ì≥Ñ Download Attachment</a></div>}
{selectedVacancy.file_url && <div style={{ marginTop: 16 }}><a href={`http://127.0.0.1:8000${selectedVacancy.file_url}`} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ display: "inline-block", fontSize: 13, padding: "6px 16px", textDecoration: "none" }}>Ì≥Ñ Download Attachment</a></div>}
                  </div>
                  ...
                  {vacancies.map(v => (
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 12, marginBottom: 8, fontSize: 12, color: "#718096" }}>
                        <span>Ì≥Ö {formatDate(v.created_at)}</span>
                        <span>Ì¥ñ {v.category}</span>
                      </div>
                      <p style={{ fontSize: 13, color: "#4a5568", lineHeight: 1.6 }}>
                        {v.content.substring(0, 120)}‚Ä¶
                      </p>
